...
class PARAFAC:
    ...
    def fit(self, eem_dataset: EEMDataset):
        ...
        try:
            if not self.non_negativity:
                if np.isnan(eem_dataset.eem_stack).any():
                    ...
                    cptensors = parafac(eem_dataset.eem_stack, rank=self.n_components, mask=mask, init=self.init)
                else:
                    cptensors = parafac(eem_dataset.eem_stack, rank=self.n_components, init=self.init)
            else:
                if np.isnan(eem_dataset.eem_stack).any():
                    mask = np.where(np.isnan(eem_dataset.eem_stack), 0, 1)
                    cptensors = non_negative_parafac(eem_dataset.eem_stack, rank=self.n_components, mask=mask, init=self.init)
                else:
                    cptensors = non_negative_parafac(eem_dataset.eem_stack, rank=self.n_components, init=self.init)
        except ArpackError:
            print(
                "PARAFAC failed possibly due to the presence of patches of nan values. Please consider cut or "
                "interpolate the nan values.")
        a, b, c = cptensors[1]
        component_stack = np.zeros([self.n_components, b.shape[0], c.shape[0]])
        ...
        if self.sort_em:
            ...
            fmax = pd.DataFrame(fmax, columns=['component {r}'.format(r=i + 1) for i in range(self.n_components)])
